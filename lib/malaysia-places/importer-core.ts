...
export async function importMalaysiaPlaces(client: SupabaseClient<Database>, city: 'Kuala Lumpur' | 'Penang' | 'Melaka', queries: string[]) {
  ...
  const rows = Array.from(new Map(candidates.map((place) => [place.externalPlaceId, ({ google_place_id: place.externalPlaceId, name: place.name, country: 'Malaysia', city, area: city === 'Kuala Lumpur' ? deriveMalaysiaPlaceArea(place.addressComponents ?? [], place.address) : null, latitude: place.latitude, longitude: place.longitude, category: place.types[0] ?? null, subcategories: place.types, opening_periods: place.openingPeriods ?? null, google_rating: place.rating, google_rating_count: place.ratingCount, price_level: place.priceLevel, photo_name: place.photo?.name ?? null, photo_width_px: place.photo?.widthPx ?? null, photo_height_px: place.photo?.heightPx ?? null, photo_attributions: place.photo?.attributions ?? [], source: 'google_places', last_verified_at: new Date().toISOString() })])).values());
