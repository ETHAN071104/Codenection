...
export function mapMalaysiaPlaceRow(
  ...
  return {
    ...
    estimatedDurationMinutes:
      typeof row.estimated_duration_minutes === 'number'
        ...
        : null,
    indoorOutdoor:
      row.indoor_outdoor === 'indoor' ||
      row.indoor_outdoor === 'outdoor' ||
      row.indoor_outdoor === 'mixed'
        ? row.indoor_outdoor
        : null,
    bestTimeOfDay:
      typeof row.best_time_of_day === 'string' ? row.best_time_of_day : null,
    openingPeriods: parseStoredOpeningPeriods(row.opening_periods),
    cultureScore:
      typeof row.culture_score === 'number' ? row.culture_score : null,
    foodScore: typeof row.food_score === 'number' ? row.food_score : null,
    natureScore: typeof row.nature_score === 'number' ? row.nature_score : null,
    shoppingScore:
      typeof row.shopping_score === 'number' ? row.shopping_score : null,
    adventureScore:
      typeof row.adventure_score === 'number' ? row.adventure_score : null,
    nightlifeScore:
      typeof row.nightlife_score === 'number' ? row.nightlife_score : null,
    photographyScore:
      typeof row.photography_score === 'number' ? row.photography_score : null,
    budgetScore: typeof row.budget_score === 'number' ? row.budget_score : null,
    googleRating:
      typeof row.google_rating === 'number' ? row.google_rating : null,
    googleRatingCount:
      typeof row.google_rating_count === 'number'
        ? row.google_rating_count
        : null,
    priceLevel: typeof row.price_level === 'string' ? row.price_level : null,
    photoName: typeof row.photo_name === 'string' ? row.photo_name : null,
    photoWidthPx:
      typeof row.photo_width_px === 'number' ? row.photo_width_px : null,
    photoHeightPx:
      typeof row.photo_height_px === 'number' ? row.photo_height_px : null,
...
export async function getCandidatePlaces(
  ...
  return (data ?? [])
    .map((row) => mapMalaysiaPlaceRow(row as Record<string, unknown>))
    .map((place) => ({
      ...place,
      ...scoreMalaysiaPlace(place, query.travelDna),
    }))
    .sort(
      (a, b) =>
        b.score - a.score ||
