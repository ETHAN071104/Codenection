...
function SkipWeatherPreview({ item, applying, error, onApply, onCancel }: { item: ItineraryItemView; applying: boolean; error: string | null; onApply: () => void; onCancel: () => void }) {
  return <div><p className="text-sm font-semibold">Skip {item.place.name}</p><p className="mt-2 text-sm leading-6 text-[#5a5d61]">Route and remaining schedule will be recalculated.</p>{error && <p className="mt-4 text-sm text-red-700">{error}</p>}<div className="mt-5 flex gap-3"><ActionButton onClick={onApply} disabled={applying}>{applying ? 'Applying' : 'Apply changes'}</ActionButton><button type="button" onClick={onCancel} disabled={applying} className="h-11 px-3 text-sm font-semibold underline-offset-4 hover:underline">Cancel</button></div></div>;
}
